...
Evidence run: `run-21-full` (source commit `3efdeeb`), CI outputs in `results/ci/run-21-full/`.

**PASS 95 · FAIL 0 · NOT TESTED 1**
...
| A05 | Android launches to a usable Dashboard | **PASS** | A:A05.dashboard_usable: pass<br>A:J04.clean_first_run: pass (cold start TotalTime=6218ms) |
| A06 | Windows launches to a usable Dashboard | **PASS** | W:A06.dashboard_usable: pass (client=(1440, 900))<br>W:J04.clean_first_run: pass (startup=5.99s) |
| A07 | Compiler warnings recorded; avoidable app warnings resolved | **PASS** | Android 0 / Windows 0 warning(s) in Release builds |
| A08 | Desktop head builds and launches; incremental effort recorded separately | **NOT TESTED** | Desktop head measured separately (see desktop section)<br>*Uno-only, after core checkpoint* |
...
| B07 | Android primary navigation reaches Dashboard / Assets / History | **PASS** | A:B07.bottom_nav_assets: pass<br>A:B12.view_history: pass<br>A:C10.persisted_after_relaunch: pass (relaunch TotalTime=3246ms)<br>A:C02.counts_updated_after_save: pass<br>*bottom navigation used for all three destinations* |
...
| C10 | Inspection remains after full process termination and relaunch | **PASS** | unit tests: 47 passed, 0 failed<br>A:C10.persisted_after_relaunch: pass (relaunch TotalTime=3246ms)<br>W:C10.persisted_after_relaunch: pass (startup=0.73s)<br>*Android: am force-stop + relaunch; Windows: process kill + relaunch* |
...
| E01 | Initial data load has an intentional loading state | **PASS** | A:E01.loading_state: pass<br>W:E01.loading: pass<br>*deterministic 'slow' repository mode* |
...
| J04 | Clean install / first run succeeds | **PASS** | A:J04.clean_first_run: pass (cold start TotalTime=6218ms)<br>W:J04.clean_first_run: pass (startup=5.99s) |
| J05 | Process kill + relaunch preserves submitted inspection | **PASS** | A:C10.persisted_after_relaunch: pass (relaunch TotalTime=3246ms)<br>W:C10.persisted_after_relaunch: pass (startup=0.73s) |
...
| J09 | Primary workflow exercised end-to-end on Android and Windows | **PASS** | A:C10.persisted_after_relaunch: pass (relaunch TotalTime=3246ms)<br>W:C10.persisted_after_relaunch: pass (startup=0.73s)<br>A:B05.success: pass<br>W:B05.success: pass |
