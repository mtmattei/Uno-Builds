...
/// <summary>
/// Single-select chip for the .NET runtime row. Visually identical to
/// <see cref="PlatformChip"/> — text label collapses into a 28×28 ink circle
/// containing the version glyph (e.g. "10" / "9") when selected.
///
/// Single-select semantics are enforced by the parent (the page wires
/// <see cref="Toggled"/> to flip the other chip off).
/// </summary>
public sealed class RuntimeChip : Control
{
    ...
    public RuntimeChip()
    {
        ...
        PointerReleased += (_, _) =>
        {
            ...
            // Clicking the unselected chip selects it.
            if (!IsSelected)
            {
                IsSelected = true;
    ...
    internal sealed class RuntimeChipAutomationPeer : FrameworkElementAutomationPeer, ISelectionItemProvider
    {
        ...
        public void AddToSelection()
        {
            ...
            if (!chip.IsSelected) { chip.IsSelected = true; chip.Toggled?.Invoke(chip, true); }
        }
