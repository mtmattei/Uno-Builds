...
### 3.7 Group Chat Screen (Variation of Chat Screen)

Group chat shares the same base layout as 1:1 chat but with these specific differences:
...
│ 1:1 CHAT HEADER                                             │
├─────────────────────────────────────────────────────────────┤
│ ← [Avatar+Status] Contact Name           📞 📹             │
│                   "Personal message" or "Online"            │
├─────────────────────────────────────────────────────────────┤
│ GROUP CHAT HEADER                                           │
├─────────────────────────────────────────────────────────────┤
│ ← [Avatar]        Group Name             📞 📹             │
...
Group avatar: No status badge, uses group emoji/icon or first letter
Subtitle: Shows member count instead of personal message
```

#### Message Bubble Differences
```
┌─────────────────────────────────────────────────────────────┐
│ 1:1 CHAT MESSAGES                                           │
├─────────────────────────────────────────────────────────────┤
│ • No sender labels needed (only 2 participants)             │
│ • Sent = right, Received = left                             │
├─────────────────────────────────────────────────────────────┤
│ GROUP CHAT MESSAGES                                         │
├─────────────────────────────────────────────────────────────┤
│ • SENDER NAME shown above each received message             │
│ • Sender name: caption size (11-12px), primary color, bold  │
│ • Margin-bottom: 2-4px between name and bubble              │
│ • Sent messages (yours): no name label, right aligned       │
│ • Group messages from same sender in sequence:              │
│   - Only show name on first message                         │
│   - Subsequent messages: reduced top margin (4px vs 8px)    │
└─────────────────────────────────────────────────────────────┘
```
...
GROUP CHAT MESSAGE LAYOUT:
         ...
         ┌──────────────────┐
         │  Your message    │  ← No name (it's you)
         └──────────────────┘
                      2:30 PM

  Sarah                        ← Sender name (primary color)
  ┌──────────────────┐
  │  Hey everyone!   │
  └──────────────────┘
         2:31 PM

  Mike                         ← Different sender
  ┌──────────────────┐
  │  What's up?      │
  └──────────────────┘
  ┌──────────────────┐         ← Same sender, no repeat name
  │  Ready to play?  │
  └──────────────────┘
         2:32 PM
```
...
```
┌─────────────────────────────────────────────────────────────┐
│ GROUP INFO (accessible via header tap)                      │
├─────────────────────────────────────────────────────────────┤
│ • Group name (editable by creator/admins)                   │
│ • Group emoji/avatar                                        │
│ • Member list with presence indicators                      │
│ • "Add member" action                                       │
│ • "Leave group" action                                      │
├─────────────────────────────────────────────────────────────┤
│ GROUP CREATION (from Buddy List "+ Add Group" or Chats)     │
├─────────────────────────────────────────────────────────────┤
│ • Group name input (with emoji picker)                      │
│ • Member selector (multi-select from contacts)              │
│ • Create button                                             │
└─────────────────────────────────────────────────────────────┘
```
...
• members: Contact[] (if group)
• contact: Contact (if 1:1)

Message model for groups:
• senderId: string
• senderName: string (cached for display)
• senderColor: string (optional, for visual distinction)
```
